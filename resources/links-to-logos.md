# Links to Logos
The following are links to some well-known logos. Take care to respect the licenses associated with each asset.

- [Containers Logo](https://github.com/containers/common/tree/main/logos)
- [PostgreSQL](https://wiki.postgresql.org/wiki/Logo)
- [GitHub](https://github.com/logos)
- [GitLab](https://about.gitlab.com/press/press-kit/)
- [Python](https://www.python.org/community/logos/)
- [Go](https://go.dev/blog/go-brand/Go-Logo/PNG/)
- [NGINX](https://commons.wikimedia.org/wiki/File:Nginx_logo.svg)
- [VueJS](https://github.com/vuejs/art)
